...
export default function LeftSidebar({ onNewPost }) {
  ...
  return (
    <aside className="sticky top-0 h-screen flex flex-col py-6 px-4 border-r border-border overflow-y-auto">
      {/* Logo */}
          ...
          >
            {({ isActive }) => (
              <>
                <Icon
                  size={20}
                  className={isActive ? "text-accent-blue" : "group-hover:text-slate-200"}
                />
      ...
      <button
        onClick={onNewPost}
        className="mt-4 flex items-center justify-center xl:justify-start gap-2 w-full px-4 py-2.5 rounded-full bg-accent-blue hover:bg-blue-500 text-white font-semibold text-sm transition-colors"
      >
        <PenSquare size={18} />
      ...
      {/* Current user badge */}
      {currentUser && (
        <div className="mt-4 flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-white/5 cursor-pointer">
